...
public final class KeyboardSwitcher implements KeyboardState.SwitchActions {
    ...
    private helium314.keyboard.keyboard.calculator.CalcInputView mCalcInputView;
    private View mKeyboardClipEditPanel;
    private android.widget.EditText mClipEditText;
    private android.widget.EditText mClipEditTrigger;
    private long mEditingClipId = -1L;
    private helium314.keyboard.latin.ClipboardHistoryManager mEditingHistoryManager;
    private KeyboardActionListener mOriginalKeyboardListener;
    ...
    /** Open the calculator: show numpad + expression bar, hide suggestion strip. */
    public void setCalculatorKeyboard() {
        if (DEBUG_ACTION) {
            Log.d(TAG, "setCalculatorKeyboard");
        }
        // Switch to numpad layout (reuses existing numpad — +,-,*,/,%,.,0-9,Enter)
        setKeyboard(KeyboardId.ELEMENT_CALCULATOR, KeyboardSwitchState.OTHER);
        mCalcInputView.open();
        // Hide suggestion strip, show calc bar instead
    ...
    public boolean isShowingCalculator() {
        return mCalcInputView != null && mCalcInputView.getVisibility() == View.VISIBLE;
    }

    public int getCalcInputViewHeight() {
    ...
    public void commitCalcResult() {
        if (mCalcInputView == null) {
            // Shouldn't happen, but just close the calculator
            mLatinIME.mHandler.post(this::setAlphabetKeyboard);
            return;
        }
    ...
    /** Called from InputLogic when Delete is pressed while calculator is open. */
    public void calcDeleteChar() {
        if (mCalcInputView != null) mCalcInputView.deleteChar();
    }

    /**
     * Called from InputLogic for every printable key press while calculator is open.
     * Returns true if the key was consumed by the calculator.
     */
    public void calcAppendChar(String text) {
        mCalcInputView.appendChar(text);
    }

    public void calcClear() {
        mCalcInputView.clear();
    }

    public void closeCalculator() {
        if (mCalcInputView != null) mCalcInputView.close();
        // Restore suggestion strip
        final int stripVisibility = mLatinIME.hasSuggestionStripView() ? View.VISIBLE : View.GONE;
        if (mSuggestionStripView != null) mSuggestionStripView.setVisibility(stripVisibility);
        if (mStripContainer != null)      mStripContainer.setVisibility(stripVisibility);
        // Post the keyboard switch to the next UI cycle.
        // Switching keyboard state during a touch-event callback can cause the IME window
        // to dismiss; posting avoids that timing issue.
        mLatinIME.mHandler.post(this::setAlphabetKeyboard);
    }

    // ── Clipboard edit panel ─────────────────────────────────────────────────

    /**
     * Called from ClipboardHistoryView when the user taps Edit on a clip entry.
     * Shows the edit panel above the full QWERTY keyboard and routes key events to it.
     */
    public void startClipEdit(long id, String text, String trigger,
            helium314.keyboard.latin.ClipboardHistoryManager manager) {
        if (mKeyboardClipEditPanel == null) return;
        mEditingClipId = id;
        mEditingHistoryManager = manager;
        if (mClipEditText != null) {
            mClipEditText.setText(text);
            mClipEditText.setSelection(mClipEditText.getText().length());
        }
        if (mClipEditTrigger != null) mClipEditTrigger.setText(trigger);
        // Apply keyboard theme background
        try {
            helium314.keyboard.latin.settings.Settings.getValues().mColors
                    .setBackground(mKeyboardClipEditPanel,
                            helium314.keyboard.latin.common.ColorType.STRIP_BACKGROUND);
        ...
        if (mKeyboardView != null) {
            mKeyboardView.setKeyboardActionListener(new KeyboardActionListener.Adapter() {
                @Override
                public void onPressKey(int c, int r, boolean s, helium314.keyboard.event.HapticEvent h) {
                    if (mOriginalKeyboardListener != null) mOriginalKeyboardListener.onPressKey(c, r, s, h);
                }
                @Override
                public void onReleaseKey(int c, boolean w) {
                    if (mOriginalKeyboardListener != null) mOriginalKeyboardListener.onReleaseKey(c, w);
                }
                @Override
                public void onCodeInput(int code, int x, int y, boolean repeat) {
                    android.widget.EditText et = focused[0];
                    if (et == null) return;
                    android.text.Editable ed = et.getText();
                    if (ed == null) return;
                    if (code == helium314.keyboard.keyboard.internal.keyboard_parser.floris.KeyCode.DELETE) {
    ...
    public void closeClipEdit() {
        ...
        if (mOriginalKeyboardListener != null && mKeyboardView != null) {
    ...
    public View onCreateInputView(@NonNull Context displayContext, final boolean isHardwareAcceleratedDrawingEnabled) {
        ...
        if (mKeyboardClipEditPanel != null) {
            mClipEditText    = mKeyboardClipEditPanel.findViewById(R.id.clipboard_edit_text);
            mClipEditTrigger = mKeyboardClipEditPanel.findViewById(R.id.clipboard_edit_trigger);
            android.widget.Button confirmBtn = mKeyboardClipEditPanel.findViewById(R.id.clipboard_edit_confirm);
            
            final helium314.keyboard.latin.common.Colors colors = helium314.keyboard.latin.settings.Settings.getValues().mColors;
            final int textColor = colors.get(helium314.keyboard.latin.common.ColorType.KEY_TEXT);
            if (mClipEditText != null) {
                mClipEditText.setTextColor(textColor);
                mClipEditText.setHintTextColor((textColor & 0x00FFFFFF) | 0x88000000);
                colors.setBackground(mClipEditText, helium314.keyboard.latin.common.ColorType.KEY_BACKGROUND);
            }
            if (mClipEditTrigger != null) {
                mClipEditTrigger.setTextColor(textColor);
                mClipEditTrigger.setHintTextColor((textColor & 0x00FFFFFF) | 0x88000000);
                colors.setBackground(mClipEditTrigger, helium314.keyboard.latin.common.ColorType.KEY_BACKGROUND);
            }
            if (confirmBtn != null) {
                confirmBtn.setOnClickListener(v -> confirmClipEdit());
