...
class ClipboardDao private constructor(private val db: Database) {
    ...
    private val cache = mutableListOf<ClipboardHistoryEntry>().apply {
        db.readableDatabase.query(
            TABLE,
            arrayOf(COLUMN_ID, COLUMN_TIMESTAMP, COLUMN_PINNED, COLUMN_TEXT, COLUMN_TRIGGER_KEY),
            null,
            null,
            null,
            null,
    ...
    fun addClip(timestamp: Long, pinned: Boolean, text: String, triggerKey: String = "") {
        clearOldClips()
        ...
        insertNewEntry(timestamp, pinned, text, triggerKey)
    }

    private fun insertNewEntry(timestamp: Long, pinned: Boolean, text: String, triggerKey: String) {
        val cv = ContentValues(4)
        cv.put(COLUMN_TIMESTAMP, timestamp)
        cv.put(COLUMN_PINNED, pinned)
        cv.put(COLUMN_TEXT, text)
        cv.put(COLUMN_TRIGGER_KEY, triggerKey)
        val rowId = db.writableDatabase.insert(TABLE, null, cv)

        val entry = ClipboardHistoryEntry(rowId, timestamp, pinned, text, triggerKey)
        cache.add(entry)
        cache.sort()
    ...
    fun getByTriggerKey(word: String): ClipboardHistoryEntry? {
        ...
        return cache.firstOrNull { it.triggerKey.isNotBlank() && it.triggerKey.equals(word, ignoreCase = true) }
    }
    ...
    companion object {
        ...
        private const val COLUMN_TRIGGER_KEY = "TRIGGER_KEY"
        const val CREATE_TABLE = """
            CREATE TABLE $TABLE (
                ...
                $COLUMN_TEXT TEXT,
                $COLUMN_TRIGGER_KEY TEXT DEFAULT ''
            )
        """
