...
class Database private constructor(context: Context, name: String = NAME) : SQLiteOpenHelper(context, name, null, VERSION) {
    ...
    override fun onUpgrade(db: SQLiteDatabase, oldVersion: Int, newVersion: Int) {
        ...
        if (oldVersion <= 2) {
            try {
                db.execSQL("ALTER TABLE CLIPBOARD ADD COLUMN TRIGGER_KEY TEXT DEFAULT ''")
            } catch (_: Exception) {
                // column already exists from a previous install — safe to ignore
            }
        }
    }
    ...
    companion object {
        ...
        fun copyFromDb(file: File, context: Context) {
            ...
            try {
                otherDb.readableDatabase.rawQuery("SELECT TIMESTAMP, PINNED, TEXT, TRIGGER_KEY FROM CLIPBOARD", null)
                    .use {
                        clipDao.clear()
                        while (it.moveToNext())
                            clipDao.addClip(it.getLong(0), it.getInt(1) != 0, it.getString(2), it.getString(3) ?: "")
                    }
            } catch (e: Exception) {
                // Fallback for older backups before TRIGGER_KEY column existed
