...
public class LatinIME extends InputMethodService implements
    ...
    /**
     * If the word currently being typed matches the trigger key of any clip, show
     * that clip in the suggestion strip.  Returns true when a trigger suggestion was shown.
     */
    public boolean tryShowTriggerClipSuggestion() {
        if (!hasSuggestionStripView()) return false;
        final String typedWord = mInputLogic.getCurrentTypedWord();
        final View triggerView = mClipboardHistoryManager.getTriggerSuggestionView(typedWord, mSuggestionStripView);
        if (triggerView != null) {
            mSuggestionStripView.setExternalSuggestionView(triggerView, false);
            return true;
        }
        return false;
    }
