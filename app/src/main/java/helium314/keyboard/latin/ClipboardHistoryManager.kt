...
class ClipboardHistoryManager(
    ...
    fun updateClip(id: Long, newText: String, newTriggerKey: String) {
        clipboardDao?.updateClip(id, newText, newTriggerKey)
    }

    fun getByTriggerKey(word: String): ClipboardHistoryEntry? =
        clipboardDao?.getByTriggerKey(word)

    /**
     * If the currently typed [word] matches a clip's trigger key, build and return the
     * suggestion view so LatinIME can put it in the strip.  Returns null when no match.
     */
    fun getTriggerSuggestionView(word: String, parent: ViewGroup?): android.view.View? {
        if (word.isBlank() || parent == null) return null
        val entry = getByTriggerKey(word) ?: return null

        val binding = ClipboardSuggestionBinding.inflate(LayoutInflater.from(latinIME), parent, false)
        val textView = binding.clipboardSuggestionText
        latinIME.mSettings.getCustomTypeface()?.let { textView.typeface = it }
        textView.text = entry.text.take(200)
        ...
        textView.setOnClickListener {
            ...
            if (ic != null) {
                ic.beginBatchEdit()
                // The trigger word may be in composing state (still being typed) or
                // already committed. Handle both cases:
                // 1) Clear composing text - if trigger was composing it disappears
                ic.setComposingText("", 1)
                // 2) If trigger was already committed, it's still before the cursor - delete it
                val before = ic.getTextBeforeCursor(word.length, 0)?.toString() ?: ""
                if (before.equals(word, ignoreCase = true)) {
                    ic.deleteSurroundingText(word.length, 0)
                }
                // 3) Insert the clip text directly
                ic.commitText(entry.text, 1)
                ic.endBatchEdit()
            } else {
                latinIME.onTextInput(entry.text)
            }
