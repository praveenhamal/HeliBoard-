...
public final class InputLogic {
    ...
    /** Returns the word currently being composed, for use by LatinIME (e.g. calculator trigger). */
    ...
    public InputTransaction onCodeInput(final SettingsValues settingsValues,
        ...
        // ── Calculator mode intercept ─────────────────────────────────────────
        // When the calculator bar is open, all numpad key presses are routed to
        // it instead of going to the text field.
        final KeyboardSwitcher ks = KeyboardSwitcher.getInstance();
        if (ks.isShowingCalculator()) {
            final int kc = processedEvent.getKeyCode();
            if (kc == KeyCode.DELETE) {
                ks.calcDeleteChar();
            } else if (kc == KeyCode.CALC_CLEAR) {
                ks.calcClear();
            } else if (kc == Constants.CODE_ENTER || kc == KeyCode.ACTION_NEXT
                    || processedEvent.getCodePoint() == '=') {
                // Enter or = → commit result
                ks.commitCalcResult();
            } else if (kc == KeyCode.ALPHA || kc == KeyCode.SYMBOL_ALPHA) {
                // Alpha key → close calculator
                ks.closeCalculator();
            } else if (kc == KeyCode.CALCULATOR) {
                // Tapping the calc button again → close
                ks.closeCalculator();
            } else {
                int cp = processedEvent.getCodePoint();
                // Some function-type keys (*, /, %) have keyCode == their ASCII value
                // but getCodePoint() returns 0. Fall back to keyCode for printable ASCII.
                if (cp <= 0) {
                    final int kc2 = processedEvent.getKeyCode();
                    if (kc2 > 0 && kc2 < 128) cp = kc2;
                }
                if (cp > 0) {
                    ks.calcAppendChar(String.valueOf((char) cp));
                }
            }
            return new InputTransaction(settingsValues, processedEvent,
                    SystemClock.uptimeMillis(), mSpaceState,
                    getActualCapsMode(settingsValues, keyboardShiftMode));
        }
        // ── End calculator intercept ──────────────────────────────────────────
    ...
    private void handleFunctionalEvent(final Event event, final InputTransaction inputTransaction,
        ...
        switch (keyCode) {
            ...
            case KeyCode.CALCULATOR:
                KeyboardSwitcher.getInstance().setCalculatorKeyboard();
                break;
    ...
    public void performUpdateSuggestionStripSync(final SettingsValues settingsValues, final int inputStyle) {
        ...
        if (suggestedWords != null) {
            // If the currently typed word matches a clip trigger key, show that clip suggestion
            // and skip normal suggestions entirely for this cycle.
            if (mWordComposer.isComposingWord() && mLatinIME.tryShowTriggerClipSuggestion()) {
                // trigger clip suggestion is now showing — nothing more to do
            } else if (!(suggestedWords.mInputStyle == SuggestedWords.INPUT_STYLE_BEGINNING_OF_SENTENCE_PREDICTION
