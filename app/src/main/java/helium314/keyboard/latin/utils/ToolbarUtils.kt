...
enum class ToolbarKey {
    ...
    PAGE_UP, PAGE_DOWN, FULL_LEFT, FULL_RIGHT, PAGE_START, PAGE_END, CALCULATOR
}
...
class TextDrawable(val text: String) : android.graphics.drawable.Drawable() {
    private val paint = android.graphics.Paint(android.graphics.Paint.ANTI_ALIAS_FLAG).apply {
        textAlign = android.graphics.Paint.Align.CENTER
        textSize = 44f
        style = android.graphics.Paint.Style.FILL
    }
    private var tint: android.content.res.ColorStateList? = null

    override fun draw(canvas: android.graphics.Canvas) {
        val bounds = bounds
        val x = bounds.centerX().toFloat()
        val y = bounds.centerY() - (paint.descent() + paint.ascent()) / 2
        paint.color = tint?.defaultColor ?: android.graphics.Color.WHITE
    ...
    override fun getIntrinsicWidth() = (paint.measureText(text) + 20).toInt()
    override fun getIntrinsicHeight() = (paint.textSize + 20).toInt()
}
